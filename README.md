...
指定されたディレクトリ（デフォルトはカレント）内の画像ファイルから再帰的にランダムに画像を選び、1枚のグリッド画像（5x5など）として結合します。サブディレクトリ内の画像も検索対象になります。

**使い方:**
```bash
# デフォルト（カレントディレクトリ以下から25枚で作成）
# 出力先: combined_images/random_grid_output.jpg
python combine_images.py

# 対象ディレクトリを指定（そのディレクトリ以下を再帰的に検索）
